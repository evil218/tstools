...
#define DFLT_UCS                        0x00D7 /* "×" */
#define DFLT_GB                         0xA1C1 /* "×" */
...
static void ucs4_to_utf16(uint32_t ucs4, uint16_t **utf16, int be);
static void utf16_to_ucs4(const uint16_t **utf16, uint32_t *ucs4, int be);

static uint16_t half_search(uint16_t dat, uint16_t dflt, int hi, const uint16_t *p);
static int utf16_swap(uint16_t *data);

int utf8_gb(const char *utf8, char *gb, size_t cnt)
{
        int wc = 0; /* word count, the number of words this fxn succeeded in converting */
...
int gb_utf16(const char *gb, uint16_t *utf16, size_t cnt, int be)
{
        ...
        while(1)
        {
                ...
                else
                {
                        gb2 <<= 8;
                        gb2 |= (uint16_t)(uint8_t)(*gb++);
                        *utf16 = half_search(gb2, DFLT_UCS, max, GB2UCS);
                        if(be) {
                                utf16_swap(utf16);
                        }
                        utf16++;
...
static void ucs4_to_utf16(uint32_t ucs4, uint16_t **utf16, int be)
{
        ...
        {
                ucs4 -= 0x00010000;
                *(*utf16) = (uint16_t)(0xD800 + ucs4 / 0x0400);
                if(be) {
                        utf16_swap(*utf16);
                }
                (*utf16)++;
                *(*utf16) = (uint16_t)(0xDC00 + ucs4 % 0x0400);
                if(be) {
                        utf16_swap(*utf16);
                }
                (*utf16)++;
