...
static void show_pesh(struct obj *obj)
{
        ...
        /* PES head */
        for(i = 0; i < rslt->PES_len - rslt->ES_len - 1; i++) {
                fprintf(stdout, "%02X ", rslt->PES_buf[i]);
        }
        fprintf(stdout, "%02X, ", rslt->PES_buf[i]);
        return;
}
...
static void show_es(struct obj *obj)
{
        ...
        /* ES */
        for(i = 0; i < rslt->ES_len - 1; i++) {
                fprintf(stdout, "%02X ", rslt->ES_buf[i]);
        }
        fprintf(stdout, "%02X, ", rslt->ES_buf[i]);
        return;
}
